<template>
  <div>
    ...
    <router-link to="/accountdetail/editaddress">
    ...
    <div class="quit" @click="quitshow=!quitshow">退出登录</div>
    <transition enter-active-class="animated bounceIn">
      <section class="quit-warn" v-if="quitshow">
        <img src="../../../components/User/img/警告.png" alt="">
        <p>是否退出登录</p>
        <div class="warn-sure" @click="hides">确认</div>
      </section>
...
<script>
  ...
  export default {
    ...
    methods: {
      hides() {
        if (this.show == true||this.quitshow == true) {
          this.show = false
          this.quitshow = false
        }
      }
...
<style scoped>
  ...
  .headportrait-div > img:nth-child(1) {
    width: 2rem;
    height: 2rem;
    border-radius: 50%;
  ...
  .headportrait-div > img:nth-child(2) {
    width: .7rem;
    vertical-align: middle;
  }
  ...
  .telephone-warn img:nth-of-type(1) {
    width: 3rem;
    height: 3rem;
  }
